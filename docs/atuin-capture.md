...
preflight. There is no nested environment entry. `just check` and CI build the
locked harness and run both native suites with bounded timeouts. Each native
platform retains its regenerated capture and runner receipts.
