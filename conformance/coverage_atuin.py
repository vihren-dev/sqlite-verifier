...
def sql_report(root: Path, check: dict[str, object]) -> dict[str, object]:
    ...
    complete = (schema is not None and migration is not None and isinstance(rows, list)
        ...
        and all(row.get("status") == "NATIVE_SQL_EXPECTATIONS_PASSED"
            and row.get("profile") == "3.46.0" and row.get("schema_objects") == 8
            ...
            and row.get("domain") == ("random-rowid boundary outside deterministic model"
                if row["case"] == "metadata-max-rowid-random" else "ordinary SQL") for row in rows))
    ...
    return {"profile":"3.46.0", "status":check["status"], "case_denominator":len(CASES),
        ...
        "scope":"ordinary native SQL mechanics; not framework invocation or proof witnesses",
        "random_rowid_case_denominator":1, "cases":rows}
