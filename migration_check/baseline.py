...
def check_baseline(path: Path, hashes: Mapping[str, str]) -> None:
    ...
    for name, digest in saved.items():
        if name.startswith("approved/") or name == "schema.sql":
    ...
    actual = {name: digest for name, digest in hashes.items() if name.startswith("approved/") or name == "schema.sql" and name in expected}
